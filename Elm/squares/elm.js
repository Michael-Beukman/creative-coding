...
var $author$project$Values$mapheight = 20;
var $author$project$Values$mapwidth = 20;
var $elm$core$Platform$Cmd$batch = _Platform_batch;
var $elm$core$Platform$Cmd$none = $elm$core$Platform$Cmd$batch(_List_Nil);
var $elm$core$Basics$round = _Basics_round;
var $author$project$Values$timebeforeupdate = 100;
var $author$project$Main$init = function (_v0) {
	...
	var tilewidth = (1 * minW) / whichwidth;
	var s = A2(
		$elm$core$Debug$log,
		'asdasdasd ',
		_Utils_Tuple3(minW, whichwidth, tilewidth));
...
var $author$project$Keys$toKey = function (keyValue) {
	var s = A2($elm$core$Debug$log, 'Key Presses: ', keyValue);
	var _v0 = A2(
...
var $author$project$Main$update = F2(
	function (msg, model) {
		if (msg.$ === 'Frame') {
			var time = msg.a;
			var t = model.timeToGo - time;
			var newtime = (t < 0) ? $author$project$Values$timebeforeupdate : t;
			var frame = (t < 0) ? A2($elm$core$Basics$modBy, 3, model.frame + 1) : model.frame;
			return _Utils_Tuple2(
				_Utils_update(
					model,
					{frame: frame, timeSoFar: model.timeSoFar + time, timeToGo: newtime}),
				$elm$core$Platform$Cmd$none);
		} else {
			var dir = msg.a;
			return (_Utils_eq(dir, $author$project$Types$Space) && model.gameOver) ? $author$project$Main$init(
				{startTime: 1.0, winHeight: model.screen.height, winWidth: model.screen.width}) : _Utils_Tuple2(model, $elm$core$Platform$Cmd$none);
		}
	});
...
var $author$project$Render$squares = F3(
	function (model, translatePos, width) {
		var x = model.timeSoFar;
		var n = 10000;
		var y = (((!x) ? 1 : x) / n) * 1;
		return _Utils_ap(
			_List_fromArray(
				[
					...
					_List_fromArray(
						[
							...
							_List_fromArray(
								[
									A2($joakin$elm_canvas$Canvas$Settings$Advanced$translate, translatePos.a, translatePos.b)
								]))
						]),
					_List_fromArray(
						[
			...
			(width < 10) ? _List_Nil : A3(
				...
				A2(
					...
					function (_v0) {
						...
						var h = _v0.b;
						return A3(
							$author$project$Render$squares,
							model,
							_Utils_Tuple2((translatePos.a + ((1 * width) / 4)) + ((w * width) / y), (translatePos.b + ((1 * width) / 4)) + ((h * width) / y)),
							width / 2);
					},
					_List_fromArray(
						[
...
var $author$project$Render$renderMap = F7(
	function (tileW, tileH, screenwidth, screenheight, width, height, model) {
		return A3(
			...
			_Utils_ap(
				...
				A3(
					$author$project$Render$squares,
					model,
					_Utils_Tuple2(screenheight / 4, screenheight / 4),
					screenheight / 2)));
	});
...
var $author$project$Main$view = function (model) {
	return A2(
		...
		_Utils_ap(
			_List_Nil,
			model.gameOver ? _List_fromArray(
				[
					A2(
					$elm$html$Html$div,
					_List_Nil,
				...
				[
					...
					_List_fromArray(
						[
							$elm$html$Html$text(
							'score: ' + $elm$core$String$fromInt(1))
						])),
					A7($author$project$Render$renderMap, model.screen.tileWidth, model.screen.tileHeight, model.screen.width, model.screen.height, $author$project$Values$mapwidth, $author$project$Values$mapheight, model)
				])));
};
